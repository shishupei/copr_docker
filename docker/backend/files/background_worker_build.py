...
from copr_backend.euler_msgbus import EulerMessageSender
...
class BuildBackgroundWorker(BackendBackgroundWorker):
    ...
    def _mark_running(self, attempt):
        ...
        for topic in ['build.start', 'chroot.start']:
            self.euler_sender.announce(topic, self.job, self.last_hostname)
    ...
    def _mark_finished(self):
        ...
        data = {"builds": [self.job.to_dict()]}
        self._update_frontend_task(data)
        self.euler_sender.announce("build.end", self.job, self.last_hostname)
    ...
    def handle_build(self):
        """ Do the build """
        self.euler_sender = EulerMessageSender(self.name, self.log)
        self._get_build_job()
